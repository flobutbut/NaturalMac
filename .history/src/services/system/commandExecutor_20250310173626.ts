...
export class MacOSCommandExecutor implements CommandExecutor {
  ...
  private isPathAllowed(command: string): boolean {
    // Vérifier si la commande opère dans un chemin autorisé
    return this.allowedPaths.some(path => command.includes(path))
  }
